...
- `config.yaml` — the single knob: profile, per-role models, implementer runtime, gate command, worktree dir, and `review.human_pr_review` (self | platform-team).
...
- `profiles/` — `software` (base), `machine-learning`, `database`, `work` (Atlassian/Bitbucket: Claude implementer, Bitbucket CI, self/platform-team release); selected by `config.yaml`'s `profile:`.
- `tests/test-scripts.sh` — smoke tests for the shell layer, incl. release refusals/rollback/happy path and both `human_pr_review` topologies over a real worktree + bare-remote topology.
...
- **`scripts/release.sh <slug> [--confirm-delta <text>]`** — the release driver; also `check-version` / `next-version` subcommands for the CalVer logic. `review.human_pr_review` selects the topology: `self` ff-merges the branch into local `main`; `platform-team` fetch-guards `origin/main` (before and after the gate), commits the bump on the branch, never merges/tags `main`.
...
- **Release preconditions** (all before anything irreversible; trap rollback after): APPROVE sentinel, gate green, `ARCHI.md` fresher than the last commit touching `scripts/ skills/ profiles/ config.yaml CLAUDE.md` (stale → `/compact`), version strictly increasing. `platform-team` additionally requires `origin/main` to be an ancestor of the release branch (re-checked after the gate) — closing the cross-release version collision.
...
`bash scripts/gate.sh` — shellcheck over all tracked `*.sh` plus the `gate.d/` hook running `tests/test-scripts.sh` (20 checks, incl. both release topologies). CI runs the gate on push. Currently green.
