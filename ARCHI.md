...
`bash scripts/gate.sh` — shellcheck over all tracked `*.sh` plus the `gate.d/` hook running `tests/test-scripts.sh` (28 checks, incl. the PR-merge release flow + `tag-after-merge`, the hermetic fan-exec suite, and the `nb-clean.sh` example-hook cases). CI runs the gate on push. Currently green.
