...
1. **Assemble artifacts, not transcripts:** fetch first (`git fetch origin --quiet`), then assemble the diff (`git diff origin/main...wt/<slug> -- ':/' ":(exclude,top)work/<slug>"`), the plan path, the accepted-deferral ledger `work/<slug>/deferrals.md` (pass `(none recorded)` if it does not exist), and nothing else, so synced handoffs, followups, and other reviewer artifacts cannot leak into the review. Reviewers must never see the implementation conversation or your own commentary on the diff.
