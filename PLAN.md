...
- **Next (top of queue — bitten twice):** `review-diff-base-origin-main` — base the `/3-review` + `codex-review.sh` diff on a freshly-fetched `origin/main`, not local `main`, so a stale local `main` can't pollute the review diff. Two consecutive live hits: `implementer-ladder` (local `main` 9 commits behind → 148 KB polluted diff) and `archi-fresh-in-review` (2 commits behind → `gate.sh`/`PLAN.md`/another unit's retro leaked in), both caught only by eye and hand-fixed with a mid-review FF (`work/exec-state/retro.md`, `work/implementer-ladder/retro.md`, `work/archi-fresh-in-review/retro.md`).
- Candidate: `codex-review-printf-dash` — quote the `printf` format args in `codex-review.sh` (≈ lines 106–109 emit `printf: - : invalid option`, silently dropping reviewer-prompt lines; recurred in `archi-fresh-in-review`'s review) (`work/exec-state/retro.md`; small-fix).
