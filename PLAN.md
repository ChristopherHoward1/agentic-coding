...
- **Next:** `review-diff-base-origin-main` — base the `/3-review` + `codex-review.sh` diff on a freshly-fetched `origin/main`, not local `main`, so a stale local `main` can't pollute the review diff. Proven live during `implementer-ladder`: local `main` was 9 commits behind, yielding a 148 KB polluted review diff caught only by eye (`work/exec-state/retro.md`, `work/implementer-ladder/retro.md`).
