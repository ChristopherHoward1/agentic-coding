...
- **Shipped:** v2026.9.4 `work/review-diff-base-origin-main` (`/3-review` + `codex-review.sh` base the review diff on freshly-fetched `origin/main`, local-`main` fallback — a stale local `main` can no longer pollute the review) · v2026.9.3 `work/archi-fresh-in-review` (`scripts/archi-fresh.sh` — ref-aware ARCHI freshness check; `/3-review` catches + heals staleness on the branch; `release.sh` delegates as backstop) · v2026.9.2 `work/implementer-ladder` (`AGENTS.md` build-discipline ladder: codex builds the leanest working diff within scope) · v2026.9.1 `work/codex-verdict-medium` (MEDIUM findings never gate the review verdict; only CRITICAL/HIGH block) · v2026.9.0 `work/exec-state` (`scripts/state.sh` — read-only per-work-unit execution-state deriver: stage/review/next_action from observable facts) · v2026.8.11 `work/work-artifact-capture` (`sync-artifacts`; reviewer diff excludes unit artifacts) · v2026.8.10 `work/gate-tool-preflight` (gate reports skipped checks; `gate.required_tools` hard-fail) · v2026.8.6 `work/codex-reviewer` (dual-vendor review) · v2026.8.7 `work/retro-stage` (`/5-retro`; marker arms next release) · v2026.8.8 `work/loop-hardening` (marker byte-identity, main-bound codex reviewer, /5-retro resume+cleanup, null-dispatch detection) · v2026.8.9 `work/ds-hygiene-hook` (opt-in DS hygiene gate hook; ML + work profiles wired). Reviewers on opus.
...
- **Next (top of queue — bitten 3×):** `codex-review-printf-dash` — quote the `printf` format args in `codex-review.sh` (≈ lines 106–109 emit `printf: - : invalid option`, silently dropping reviewer-prompt lines). Recurred in `exec-state`, `archi-fresh-in-review`, and `review-diff-base-origin-main` reviews — a trivial quoting bug degrading every reviewer prompt (`work/exec-state/retro.md`, `work/review-diff-base-origin-main/retro.md`; small-fix).
- Candidate: `codex-reviewer-reasoning-effort` — raise the codex `reviewer.command` reasoning effort above `none`; a no-reasoning reviewer produced a shallow false-positive HIGH (three-dot merge-base) that cost a round + Owner escalation in `review-diff-base-origin-main` (`config.yaml`/`reviewer.command`; `work/review-diff-base-origin-main/retro.md`).
- Candidate: `archi-fresh-polish` — fix the `/3-review` step-5 reference in the ARCHI-heal step (mislabeled "step 4 / step-19") and add the missing-*source* exit-2 test for `archi-fresh.sh` (`work/archi-fresh-in-review/retro.md`; small-fix).
...
- 2026-09-08 — Handling a reviewer CRITICAL/HIGH the orchestrator believes is wrong: analyze it on the merits, weigh the *other* reviewer's independent severity on the same behavior, escalate the open finding to the Owner, and override only with the analysis recorded in the plan's Review section — never ship a regression to appease a reviewer. First exercised rejecting codex's three-dot-merge-base false positive — `work/review-diff-base-origin-main/retro.md`
...
- 2026-09-08 — The orchestrator keeps the primary checkout on a clean `main`; transient status notes (e.g. a "now implementing" `PLAN.md` edit) never live uncommitted in the primary tree. Both `/4-release` (`main must be checked out in the primary worktree`) and branch switches require it, and a stale feature-branch checkout also stale-bases the review diff — self-inflicted friction that recurred across two units before earning this rule — `work/archi-fresh-in-review/retro.md`
