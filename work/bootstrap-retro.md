...
## Candidate improvements (logged, not scheduled)

Surfaced 2026-08-15 while dissecting the TRIP workflow the framework drew from. Each earns its way in only when real use demands it.

1. **Codex-as-reviewer for cross-vendor decorrelation.** Today both reviewer subagents are Claude (the `.claude/agents/` mechanism is Claude-only); codex is wired only as the *implementer*. This makes orchestrator+reviewers a correlated ensemble (see `PLAN.md` risks). A read-only codex reviewer would genuinely decorrelate the validators — but needs plumbing that doesn't exist (agent-exec.sh is write-oriented, returns a diff not a verdict) and must review units it didn't write to preserve writer≠reviewer. Blocked on mechanism; revisit if the Claude reviewers prove too agreeable in practice.
2. **Batching in `/2-implement`.** TRIP splits an approved plan into ordered batches (`Luna → Fable → Luna → Fable`) with continuous inner-loop review. This repo dispatches the whole plan to codex in one worktree run, with `gate.sh` (deterministic, not an LLM) as the only inner-loop check. Fine for small units; a gap for large ones. Add batching when a unit is too big for one clean codex dispatch.
4. **Handoff must be self-contained — uncommitted work-unit files don't reach the worktree.** First dogfood (2026-08-15) hit this: the worktree branches from HEAD, so an uncommitted `work/<slug>/plan.md` is invisible to the cold implementer, but `handoff.tpl` points at it as "source of truth." Worked around by embedding the full plan into `handoff.md`. Fix options for `/2-implement`: either commit the work unit before `worktree.sh add`, or make handoff rendering always embed the plan body (not a path). The template's own "must be self-contained" rule argues for embedding. Cheap, do it soon.

3. **A `/4-release` stage (move closer to TRIP).** TRIP formalizes version bump → changelog → docs/ARCHI sync → commit → tag → ff-merge → push, and **Fable (the orchestrator role) performs the merge** after Sol's independent APPROVED — no human in the loop. This repo currently stops at "Owner merges — the Owner's call, always" (`CLAUDE.md`). Owner (2026-08-15) prefers moving closer to TRIP here. **This changes a stated invariant** (human-gated merge → orchestrator-merges-after-review-gate), so it needs its own work unit and an explicit CLAUDE.md amendment, not a silent adoption. See [[trip-release-stage-preference]].
